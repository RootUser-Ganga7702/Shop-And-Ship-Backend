...
const productSchema = new mongoose.Schema({
  ...
  productOrderUniqueId: { type: String, unique: true },
  countryOrderId: { type: String, required: true },
  barcode: { type: String },
  ...
  courierServiceType: { type: String },
...
/**
 * 🔹 Auto-generate orderId (ORD00001)
 */
orderSchema.pre('save', async function (next) {
  ...
  /**
   * 🔹 FIXED: productOrderUniqueId generation for embedded products
   */
  try {
    const lastOrderWithProduct = await mongoose.model('Order').findOne(
      { 'productsList.productOrderUniqueId': { $exists: true } },
      { productsList: 1 }
    ).sort({ createdAt: -1 });
    ...
    if (lastOrderWithProduct) {
      lastOrderWithProduct.productsList.forEach(p => {
        if (p.productOrderUniqueId) {
          const num = parseInt(p.productOrderUniqueId.replace('PORD', ''), 10);
          if (!isNaN(num) && num >= nextProductNumber) {
            nextProductNumber = num + 1;
          }
        }
      });
    }
    ...
    this.productsList.forEach(product => {
      if (!product.productOrderUniqueId) {
        product.productOrderUniqueId = `PORD${String(nextProductNumber).padStart(5, '0')}`;
        nextProductNumber++;
      }
    });
  } catch (error) {
    console.error('Error generating productOrderUniqueId:', error);
  }
...
// productSchema.index({ userId: 1, orderStatus: 1 });

// productSchema.pre('save', async function (next) {
//   if (!this.productOrderUniqueId) {
//     try {
...
//       if (lastOrder && lastOrder.productOrderUniqueId) {
//         const lastNumber = parseInt(
//           lastOrder.productOrderUniqueId.replace('PORD', ''),
//           10
//         );
//         if (!isNaN(lastNumber)) nextNumber = lastNumber + 1;
//       }

//       this.productOrderUniqueId = `PORD${String(nextNumber).padStart(5, '0')}`;
//     } catch (error) {
//       console.error('Error generating productOrderUniqueId:', error);
//     }
//   }
